...
function [g, dg] = workerUpdate(param, x, y, M)
[N,D] = size(x);
sn2 = exp(2*param(1));
...
% df_dlnsn
dg(1) = N-1/sn2*(y'*y-2*y'*Phin*m+sum(sum(PhinPhin.*S_mm))+...
  sf2*N-sum(sum(Phin.*Phin)));

% df_dlnell
% dg(2:D+1) = -eta.*(2*ones(1,M)*(B.*(cholInvKbbEDotKbx*x))-...
...
% df_dlnsf
dg(D+2) = 1/sn2*(-y'*Phin*m+sum(sum(PhinPhin.*S_mm))+...
  sf2*N-sum(sum(Phin.*Phin)));

% df_dB
dg(D+3:D+2+M*D) = reshape((cholInvKbbEDotKbx*scale_cols(x,eta)-...
  (cholInvKbbEDotKbx*ones(N,1)*ones(1,D)).*scale_cols(B,eta)-...
  FF*scale_cols(B,eta)+(FF*ones(M,1)*eta').*B)/sn2, M*D, 1);

% df_dm
dg(D+3+M*D:M+D+2+M*D) = -y'*Phin/sn2+m'*PhinPhin/sn2;

% df_dcholS
cholS_df_dS = cholS*PhinPhin/sn2;
dg(M+D+3+M*D:M+D+2+M*D+M*(M+1)/2) = reshape(cholS_df_dS(upper_ind),M*(M+1)/2,1);
end